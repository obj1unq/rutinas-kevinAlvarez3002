...
class PersonaBase {
  ...
  var property peso  
  method pesoQuePierdeAlPracticarRutina(rutina) {
return rutina.caloriasQueBaja(tiempoQueEjercita)/kilosPorCaloria
  }
...
class PersonaAtletica inherits PersonaBase(kilosPorCaloria=8000 ,tiempoQueEjercita=90) {
    override method practicarRutina(rutina){
...
class Club {
    ...
    method mejorPredioParaPersona(persona) = predios.max({predio=> predio.caloriasQueGastaria(persona)}) 
    method prediosTranquis(persona) =predios.filter({predio=>predio.tieneRutinaTranqui(persona)}) 
    method rutinasMasExigentes(persona) =predios.map({predio=>predio.rutinaMasExigente(persona)}).asSet()
}
class Predio{
    const property rutinas =#{} 
    method caloriasQueGastaria(persona) = rutinas.sum({rutina=>persona.pesoQuePierdeAlPracticarRutina(rutina)})
    method tieneRutinaTranqui(persona) = rutinas.any({rutina=>persona.caloriasQueBajaria(rutina)<500})//faltaria crear un metodo en base a lo del bloque 
    method rutinaMasExigente(persona) =rutinas.max({rutina=>persona.caloriasQueBajaria(rutina)})
}